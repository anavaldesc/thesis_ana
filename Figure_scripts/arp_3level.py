...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 16 10:44:46 2019
...
def H_rf(Omega, delta, epsilon):
    
    H = [[-delta, Omega/2, 0],[Omega/2, -epsilon, Omega/2],[0, Omega/2, delta]]
...
delta_resampled = np.linspace(delta.min()-83, delta.max()-83, 200)
...
#    plt.plot(delta_resampled, e0[:,i], 'k--')
    if i == 1:
        ...
        color = pops[:, i,0]
    plt.scatter(delta_resampled+83, energies[:, i], c=color, s=0.5, cmap='bwr')
plt.xlabel('$\delta$ [kHz]')
plt.ylabel('Energy [kHz/$h$]')
plt.xlim([delta.min(), delta.max()])
plt.tight_layout()
