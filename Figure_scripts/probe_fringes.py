...
plt.pcolormesh(x, y, od, cmap='seismic', vmax=0.75, vmin=-0.75, rasterized=True)
ax.set_aspect('equal')
...
ax = plt.subplot(gs[0,0])
plt.pcolormesh(x, y, bad_od, cmap='seismic', vmax=0.75, vmin=-0.75, rasterized=True)
plt.xticks([])
plt.yticks([])
plt.text(0,644+30,'$\mathbf{a.}$ Flea3 camera')
ax.set_aspect('equal')
plt.tight_layout()
plt.savefig('probe_comparison.pdf', dpi=1000)