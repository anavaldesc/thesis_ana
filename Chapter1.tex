...
\chapter{Quantum degenerate gases}

Qantum degeneracy is reached when an ensemble of particles in equilibrium maximally occupies a given quantum state. Bosons, who are famous for liking each other, will bunch together to fill such a state while fermions, in a more discriminatory fashion, will occupy different states according to the Pauli exclusion principle.

Quantum degeneracy has been experimentally reached in a couple of systems \cite{}, most notably in dilute clouds of very cold atoms \cite{}. A mostly unambiguous measurement of quantum degeneracy is given by a larger than unity phase-space density, where for atomic vapors the phase-space quadratures are in the motional degrees of freedom (position and momentum). To better quantify this, take an alkali atom (first column of your favorite periodic table) as an example; atomic transitions display energies of a few \units{}{}{eV}, roughly a part in $10^{22}$ of the energy needed to recharge the battery of a contemporary electronic device, which sets the natural energy scale of the problem. The temperatures at which these atomic vapors are able to reach quantum degeneracy are on the order of a couple \units{}{100}{nK}, which itself corresponds to a trillionth of the atomic energy scale. The density of atoms in these gases is typically of a few \units{}{10^{15}}{cm^{-3}}, which compared to the density of air at standard atmospheric conditions (room-temperature and sea level) is $10^{5}$ times more dilute. While these scales vastly undershoot every-day experience, the characteristic phase-space density of a quantum degenerate gas overshoots it by approximately $10^{10}$. Finally, in therms of their aspect, they resemble a ~\units{}{10}{\mu m} blob, close to the actual size of a white-blood cell or a tenth of the width of a human hair. In the lab, they are kept floating around inside an ultra high vacuum (UHV) environment, isolated from everything but the occasional presence of light.

\section{Cold atoms}

An extensive list of studies on cold atomic systems \cite{} can show how the pioneering work by X, Y, Z and others \cite{} has revolutionized quantum physics in recent times. Originally motivated by the advancement of precision measurement in the context of laser spectroscopy and accurate time-keeping \cite{}, the field has exploded into the more contemporary branches of quantum information and quantum simulation. Nevertheless, the ongoing development of both experimental and theoretical tools for cold atomic systems has made substantial contributions to the fields of statistical mechanics \cite{}, condensed-matter physics \cite{}, metrology \cite{}, optics \cite{}, chemistry \cite{}, among others. Cold neutral atoms serve as the starting point in the production of quantum degenerate gases, experiments on which constitute the main subject of this thesis. 

Laser cooling \cite{} is the method used to produce cold samples of neutral atoms in the lab. The technique has been reviewed extensively \cite{} and the specific details of the lab implementation that is relevant to this thesis have also been documented \cite{}. What this thesis contains is an author's inspired example of the various concepts revolving around laser cooling techniques and an in-depth analysis is presented.

One of the simplest forms of light-matter interaction is one involving a simple two-level atom and a scalar electromagnetic (EM) plane wave. The interaction Hamiltonian is the dipole interaction 

\begin{equation}
	H_{d} = -e\,\vec{\rm{E}}(\vec{r}, t) \cdot \vec{r}, \nonumber\\
\end{equation}

with ${e}$ the charge of an electron and ${\vec{\rm{E}}(\vec{r}) = \rm{E}_0 \hat{\epsilon} \cos{(\vec{k}\cdot\vec{r} - \omega\, t)}}$ the electric field describing a plane wave with polarization along ${\hat{\epsilon}}$.

\section{Making a cold atomic gas}

XX Talk about the employed methods to laser cool

\section{Study: vector laser-cooling}

XX Talk about space-dep polarization gradient cooling

\section{Ultracold atoms}

While laser cooling can increase phase-space density to approach degeneracy, it is usually insufficient (though there exist ongoing efforts in laser cooling all the way to degeneracy \cite{•}). In the experiments described here, degeneracy is reached by means of a combination of Doppler-limited, sub-Doppler-limited laser cooling and forced evaporative cooling. 

\section{Making a quantum degenerate Bose gas}

XX Talk about the employed methods to get BEC

\section{Study: degeneracy beyond the ground state}

XX Talk about getting a BEC in say, the first excited state of a SHO.

It's always good to have Maxwell's equations \cite{Agrawal2,Diament}, 
%1.5
\begin{eqnarray} 
