...
{\bf Toptica's BoosTA:} Our cooling light comes from a \noted{Toptica DL Pro} is amplified using a \noted{Toptica BoosTA} tapered amplifier system. The output power of this TA has been relatively stable over the years, in fact it has been operating for over 7 years now unlike other homemade TA setups (see~\cite{{PriceThesis,CampbellThesis}}) where we had to replace the TA chips multiple times. The downside is it has a tendency to turn itself off and on its worst days it would turn off so often that it would be impossible to operate the experiment. We haven't been able to identify the problem despite our best efforts to look into the TA controller, the TA itself, multiple conversations with Toptica engineers etc. 
