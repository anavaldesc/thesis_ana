...
\chapter{Engineering quantum matter with unconventional topology}
...
Topology is quantified by integer valued topological invariants, for example the Chern number or a Z2 invariant.  We break from this mold: using laser coupled 87Rb atoms, we engineer 2D spin orbit coupling (SOC) –related to Rashba SOC– and experimentally show that the resulting topological invariants can take on both integer and half-integer values.  
Our advance combines a key insight obtained in the context of atmospheric waves, where topology was found in systems without any lattice, with the experimental and measurement technology present in cold quantum gases.  Our manuscript is divided into two sections: 

This Chapter is divided into two section. First I will give a general overview of topological order in Physics and its application to band theory of solids. In the second section I will describe a series of experiments that we performed to engineer and characterize the topology of a Rashba spin-orbit coupled quantum gas. This section is divided into two parts: first we describe the engineering of the Rashba Hamiltonian using a trio of Raman coupled CDD states (Chapter~\ref{ch:clock_states}) and validate our engineering using Fourier transform spectroscopy (Chapter~\ref{ch:Fourier_spectroscopy}). Finally we describe a quantum state tomography procedure to measure the full quantum state of our system, and directly obtain the Chern number for both a topologically trivial, where it is zero, and non-trivial configurations, where it takes half integer values. 
...
Condensed matter classifies phases of matter. Phases can sometimes be described in terms of symmetries that are spontaneously broken. Recently a new kind of order has been found that there are distinct phases of matter that do not  

\subsection{Topology?}

Topology is a branch of mathematics that studies the properties of geometric objects that are conserved under continuous deformation. Two objects that can be continuously deformed into each other are said to be homeomorphic and can be classified by the same integer-valued topological invariant\note{Look up a good reference for topology in math}. One easy way of illustrating this is to think about closed two-dimensional surfaces in three dimensions. A donut can be continuously deformed into a coffee mug but if we want to deform it into a pretzel we need to poke more holes in it. The donut and the mug must share the same topological invariant, which is different from that of the pretzel. More formally, these solids are classified by the Euler characteristic which is related to the integral of of the Gaussian curvature $K$ over the whole surface by the Gauss-Bonnet theorem:
%
\begin{equation}
	\chi = \frac{1}{2\pi}\int_S K dA.
\end{equation}
%
We can find a normal vector at any point of the surface which which is orthogonal to the surface. We can then define a plane containing the normal vector and its intersection with the surface defines a curve. The curvature of this curve is called the normal curvature $\kappa$. The minimum and maximum of these are called the principal curvatures and are used to define the Gaussian curvature at any point of a surface $K=\kappa_{min}\kappa_{max}$. For closed surfaces $\chi=2(1-g)$ where the genus $g$ represents the number of holes or handles in the surface. 
...
% The effects of topology in condensed matter in Bloch bands was first observed by Von Klitzing~\cite{klitzing_new_1980} when he measured quantized conductivity for 



Topological band theory classifies topologically distinct Hamiltonians $\hat{H}(\k)$

to David J. Thoules, F. Duncan M. Haldane and J. Michael Kosterlitz for theoretical discoveries of topological phase transitions and topological phases of matter. Kosterlitz and Thoules first used topology to describe phase transition on 2D materials~\cite{kosterlitz_long_1972}. A decade later Thoules, Kohomoto, Nightingale, and den Nijs explain used the topology arguments to explain the Quantum Hall effect~\cite{thouless_quantized_1982}. In their paper they showed that the quantization of the Hall conductivity
%
\begin{equation}
	\sigma_{xy} = Ne^2/h
...
\end{equation}
%
is determined by the underlying topology of the band structure. Just like the genus classifies 2D solids that can be continuously deformed without opening or closing holes, the `TKNN' invariant classifies band structures that can be deformed into one another without closing an energy gap. The mathematical physicist Bary Simon recognized this invariant as the `first Chern class invariant from $U(1)$ fiber bundles'~\cite{simon_holonomy_1983} (see~\cite{geometry_topology_physics} for example if you want to learn more about this subject). The other very valuable contribution was that he made the connection between the Chern number and the the Berry's geometrical phase~\cite{berry_michael_victor_quantal_1984} which allows us to make a physical interpretation of this otherwise abstract seeming invariant. In the following sections I will describe important concepts in 
...
Consider a Hamiltonian that depends on a set of parameters $\mathbf R$ $\hat{H}(\mathbf{R}$ ~\cite{berry_michael_victor_quantal_1984}
