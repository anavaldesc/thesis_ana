...
(1) in the first we demonstrate our approach for reliably creating our 2D SOC, and use time-domain spectroscopy to directly measure the resulting spectrum: essentially validating our quantum engineering.  (2) In our second section we employ quantum state tomography to measure the full quantum state of our system, and directly obtain the Chern index, both in a topological trivial configuration, where it is zero, and in a topological configuration where it is one-half.
The ubiquitous nature of topology in an ever widening array physical systems makes our work of interest to a wide audience, where topology is both of fundamental and practical importance.  Our present work clearly shows new non-integer values for topological invariants, but leaves open the “bulk-boundary” connection, which links quantized transport to interfaces between systems with different topological invariants.


\label{ch:Rashba}

\section{Overview of topological band theory}
