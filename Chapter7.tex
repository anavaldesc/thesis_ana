...
\end{equation}
%
which can be interpreted as the number of times that the vector $\hat{h}(\k)$ wraps around a unit sphere~\cite{kaufmann_notes_2016}, a quantity that is known as the winding number. Depending on the sign of $h_z$ the vector $\hat{h}(\k)$ will wrap around the north or south hemisphere, so in order to have integer valued Chern numbers it is necessary that Dirac points come in pairs. Luckily for time reversal invariant materials this is guaranteed by the fermion doubling theorem~\cite{nielsen_adler-bell-jackiw_1983}. In Chapter~\ref{ch:Rashba} I describe an engineered system that has a single Dirac point. 
