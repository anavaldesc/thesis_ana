...
\end{equation}
%
\note{TODO:add the fudge factors that give more weight to one of the transitions (both here and in Gamma)}
...
The vector light shift enables the realization of various spin dependent potentials in the lab. In the experiments presented in Chapters~\ref{ch:Fourier_spectroscopy} and \ref{ch:Rashba} I used combinations of cross polarized laser beams such that for the total electric field $\mathbf{E}^*\times\mathbf{E}\neq0$ to induce Raman tranisitions. A Raman transition is a two-photon process between two ground states that uses an intermediate state that is off-resonantly coupled as is shown in Figure~\ref{fig:Raman_coupling}a. Typically the intermediate state is far detuned and is adiabatically eliminated so the system is effectively treated as a two level system, 

In our case we couple the $m_F$ levels of the $F=1$ manifold using light that is off-resonant with any of the $5P$ transitions. We typically used lasers with angular frequencies close to the magic wavelength $\lambda_{\mathrm{R}}=790.032\,\nm$ described in Section~\ref{sec:scalar_light_shift} such that the scalar light shift vanished. Because the Raman laser beams were considerably detuned from the D1 and D2 lines we do not take into account coupling into any of the $5P$ levels\footnote{Some atoms are inevitably excited of course, leading to heating from spontaneous emission (Equation~\ref{eq:scattering})}


I consider atoms in the $F=1$ manifold in the presence of a magnetic field along $\ez$ such that there is a non-negligible quadratic Zeeman shift. Two laser beams counter propagating along $\ex$ and with polarizations along $\ey$ and $\ez$ give rise to an effective magnetic field along $\ex$. 

which can be generalized to more levels and different geoemtries 
...
\caption[Raman coupling with two-photon transitions]{The Rashba dispersion relation has a Dirac point located at $\k=0$ and }
