...
%
\begin{equation}
	\boldsymbol{\hat \mu}=-g_J\mu_B \mathbf{\hat J}+g_I\mu_N\mathbf{\hat I}=\mu_B g_F \mathbf{\hat F}
\end{equation}
%
where $\mu_B$ is the Bohr magneton, $\mu_N$\footnote{$\mu_N\ll\mu_B$ and the nuclear contribution is usually neglected.} is the nuclear magneton and $g_J$, $g_I$ and $g_F$ are the Land\'e $g$-factors corresponding to the electronic, nuclear and total angular momentum\footnote{g_I is small compared to . In the presence of an external magnetic field $\mathbf B$, the internal levels of an atom get modified due to the Zeeman~\cite{Zeeman_effect} interaction
%
\begin{equation}
...
The total energy shifts are calculated by diagonalizing the full atomic Hamiltonian including the fine and hyperfine structure terms. For a small applied magnetic the energy split is linear with the magnitude of the field $\Delta E_{\rm{Zeeman}}=g_F\mu_B m_FB$
