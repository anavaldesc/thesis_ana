...
\end{equation}
%
where $\delta=\omega-\omega_{ge}$ is a detuning and $\tilde{\Omega}=\sqrt{\Omega^2+\delta^2}$ is known as the generalized Rabi frequency. Figure~\ref{fig:rabi_cycle} shows an example of a Rabi oscillation between the $m_f=-1$ and $m_f=0$ states driven with an RF field oscillating with frequency close to the Zeeman splitting between the states.  
